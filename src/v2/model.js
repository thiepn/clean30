...
export const roomTypes = [
  {
    ...
    label: "Kitchen",
    icon: "kitchen",
    defaultFeatures: ["oven", "microwave", "refrigerator"],
    features: [
      ["oven", "Oven"],
      ["microwave", "Microwave"],
  ...
  },
  {
    ...
    label: "Bathroom",
    icon: "bathroom",
    defaultFeatures: ["toilet", "shower", "mirror"],
    features: [
      ["toilet", "Toilet"],
      ["shower", "Shower"],
      ["bath", "Bathtub"],
...
export const taskCatalog = [
  task("kitchen-dishes", "kitchen", "Wash dishes and clear the sink", 7, 10, { guest: true, essential: true, defaultEnabled: true }),
  task("kitchen-counters", "kitchen", "Clean counters and backsplash", 7, 30, { guest: true, essential: true, defaultEnabled: true }),
  task("kitchen-sink", "kitchen", "Scrub the sink and faucet", 7, 35, { guest: true, essential: true }),
  task("kitchen-stovetop", "kitchen", "Clean the stovetop", 7, 40, { essential: true }),
  ...
  task("bathroom-sink", "bathroom", "Clean the sink, faucet, and counter", 7, 30, { guest: true, essential: true, defaultEnabled: true }),
...
export function buildConfiguredTasks(rooms, cleanDays = [2, 4, 6], startKey = dateKey()) {
  ...
  let monthlyIndex = 0;
  return all.map((item) => {
    const selectedIndex = item.cadence <= 14 ? weeklyIndex++ : monthlyIndex++;
    const cadenceOffset = item.cadence <= 14
      ? selectedIndex % Math.max(1, cleanDays.length)
      : selectedIndex % Math.max(4, cleanDays.length * 4);
    return {
      ...item,
      enabled: item.defaultEnabled,
      nextDue: nextSelectedDay(startKey, cleanDays, cadenceOffset)
    };
  });
}
...
export function createCustomTask(room, title, cadence = 7, cleanDays = [2, 4, 6], startKey = dateKey()) {
  ...
  return {
    ...
    cadence: Math.max(1, Number(cadence) || 7),
    stage: 50,
    defaultEnabled: true,
    enabled: true,
    feature: null,
    ...
    detail: "",
    custom: true,
    nextDue: nextSelectedDay(startKey, cleanDays)
  };
}
...
export function normalizeV2State(value) {
  ...
  const tasks = Array.isArray(value.tasks)
    ? value.tasks.filter((item) => item && item.id && validRoomIds.has(item.roomId)).map((item) => ({
        ...
        cadence: Math.max(1, Number(item.cadence) || 7),
...
export function loadV2StateResult() {
  try {
    const saved = window.localStorage.getItem(V2_STORAGE_KEY);
    return { state: saved ? normalizeV2State(JSON.parse(saved)) : createFreshV2State(), error: null };
  } catch {
    return { state: createFreshV2State(), error: "Clean30 could not read saved data. A fresh local state was opened." };
  }
}
...
export function buildWeeklyReset(state) {
  return (state.tasks || [])
    .filter((item) => item.enabled && item.cadence <= 14 && (item.essential || item.custom))
...
export function buildRoomPlan(state, roomId) {
  return (state.tasks || [])
    .filter((item) => item.enabled && item.roomId === roomId)
  ...
  return normalizeV2State({
    ...
    tasks: state.tasks.map((item) =>
      completedIds.has(item.id)
        ? { ...item, nextDue: nextSelectedDay(addDays(completedKey, item.cadence), state.cleanDays) }
        : item
    ),
