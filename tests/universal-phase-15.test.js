    ...
    phases: [
      {
        id: `${id}-phase`,
        title: room,
        tasks: [{ id: `${id}-task`, title: `Clean ${room}` }]
      }
    ]
  };
}
...
test("a custom Task Library item remains selectable when it duplicates a built-in title", () => {
  ...
  const resolved = items.find(
    (item) => item.room === "Bathroom" && item.title === "Clean the toilet"
  );
...
test("freshness labels cannot contradict the room-care state near a threshold", () => {
  const recent = getRoomFreshnessPresentation({
    status: "recent",
    daysSince: 6,
    suggestedIntervalDays: 10
  });
  const soon = getRoomFreshnessPresentation({
    status: "soon",
    daysSince: 8,
    suggestedIntervalDays: 10
  });
...
test("archived routine activity can still describe which rooms that routine covers", () => {
  const routine = roomRoutine({ archived: true });
  assert.equal(
    routineCoversRoom(routine, "Kitchen", { includeArchived: true }),
    true
  );
  assert.deepEqual(
    getRoutineCoveredRooms(routine, ["Kitchen", "Bathroom"]),
    ["Kitchen"]
  );
});
