...
# ---------------------------------------------------------------------------
# storage.js — full-backup v4 + canonical maintenance state
# ---------------------------------------------------------------------------
storage_path = Path("src/utils/storage.js")
storage = storage_path.read_text()

anchor = '} from "./templateUtils.js";\n'
if anchor not in storage:
    raise SystemExit("Missing storage import anchor")
...
before = '''export function normalizeAppState(value) {
  if (!isPlainObject(value)) return createLegacyState();
'''
after = '''function normalizeMaintenanceTasksByTemplate(value, templates) {
...
'''
if before not in storage:
    raise SystemExit("Missing normalizeAppState anchor")
storage = storage.replace(before, after, 1)

before = '''  const todayTasksByDate = normalizeTodayTasksByDate(
  ...
  return {'''
after = '''  const todayTasksByDate = normalizeTodayTasksByDate(
    value.todayTasksByDate,
    activeTemplate,
    dailyRuleCompletions,
    value.dashboardTodos,
  ...
  return {'''
if before not in storage:
    raise SystemExit("Missing Today normalization anchor")
storage = storage.replace(before, after, 1)

before = '''    dailyRuleCompletions,
    todayTasksByDate,
    dashboardTodos: normalizeDashboardTodos(value.dashboardTodos),'''
after = '''    dailyRuleCompletions,
    todayTasksByDate,
    maintenanceTasksByTemplate,
    maintenanceCompletions,
    dashboardTodos: normalizeDashboardTodos(value.dashboardTodos),'''
if before not in storage:
    raise SystemExit("Missing normalized state return anchor")
storage = storage.replace(before, after, 1)

before = "  const state = saved ? normalizeAppState(saved) : createLegacyState();"
after = '''  const state = saved
    ? normalizeAppState(saved)
    : normalizeAppState(createLegacyState());'''
if before not in storage:
    raise SystemExit("Missing loadAppState anchor")
storage = storage.replace(before, after, 1)

before = '''const CURRENT_STATE_FIELDS = [
  "templates",'''
validators = '''const CURRENT_MAINTENANCE_TASK_FIELDS = [
  "id",
  "catalogId",
  "title",
  "room",
...
const CURRENT_STATE_FIELDS = [
  "templates",'''
if before not in storage:
    raise SystemExit("Missing current state fields anchor")
storage = storage.replace(before, validators, 1)
...
after = '''  "dailyRuleCompletions",
  ...
  "dashboardTodos",'''
if before not in storage:
    raise SystemExit("Missing state field list maintenance anchor")
storage = storage.replace(before, after, 1)

before = '''  if (!isValidTodayTasksByDate(data.todayTasksByDate)) {
    return "Current backup contains invalid or incomplete Today task data.";
  }
'''
after = '''  if (!isValidTodayTasksByDate(data.todayTasksByDate)) {
    return "Current backup contains invalid or incomplete Today task data.";
  }
  if (
...
'''
if before not in storage:
    raise SystemExit("Missing Today backup validation anchor")
storage = storage.replace(before, after, 1)

# Only the explicit reset function needs canonical fields; loadAppState normalizes legacy constructors.
reset_marker = "export function resetToFreshState() {"
reset_index = storage.find(reset_marker)
if reset_index < 0:
    raise SystemExit("Missing resetToFreshState")
reset_chunk = storage[reset_index:]
before = '''    todayTasksByDate: {},
    dashboardTodos: [],'''
after = '''    todayTasksByDate: {},
    maintenanceTasksByTemplate: {
    ...
    maintenanceCompletions: [],
    dashboardTodos: [],'''
if before not in reset_chunk:
    raise SystemExit("Missing resetToFreshState Today anchor")
reset_chunk = reset_chunk.replace(before, after, 1)
storage = storage[:reset_index] + reset_chunk
storage_path.write_text(storage)


# ---------------------------------------------------------------------------
# App.jsx — collect task-level completion events from current cleaning flows
# ---------------------------------------------------------------------------
app_path = Path("src/App.jsx")
app = app_path.read_text()

before = 'import { mergeHomeRoomsWithZones } from "./utils/homeLibrary.js";\n'
after = '''import { mergeHomeRoomsWithZones } from "./utils/homeLibrary.js";
import {
...
if before not in app:
    raise SystemExit("Missing App maintenance import anchor")
app = app.replace(before, after, 1)

before = '''  function finishSession() {
...
if before not in app:
    raise SystemExit("Missing finishSession anchor")
app = app.replace(before, after, 1)

before = '''  function toggleTodayTask(taskId) {
...
after = '''  function toggleTodayTask(taskId) {
    const dateKey = getTodayKey();
    setAppState((current) => {
...
if before not in app:
    raise SystemExit("Missing toggleTodayTask anchor")
app = app.replace(before, after, 1)

before = '      message: "This clears completed sessions but keeps templates and Today tasks.",'
after = '      message: "This clears completed sessions and cleaning-task history, but keeps your setup and Today tasks.",'
if before not in app:
    raise SystemExit("Missing reset history copy")
app = app.replace(before, after, 1)

before = '        setAppState((current) => ({ ...current, history: [] }));'
after = '''        setAppState((current) => ({
          ...current,
          history: [],
          maintenanceCompletions: []
        }));'''
if before not in app:
    raise SystemExit("Missing reset history state anchor")
app = app.replace(before, after, 1)

before = '''        setAppState((current) => ({
          ...current,
...
if before not in app:
    raise SystemExit("Missing deleteHistoryEntry anchor")
app = app.replace(before, after, 1)
...
verifier_path = Path("scripts/verify-release.mjs")
verifier = verifier_path.read_text()
for before, after, label in [
    ("/app-shell-v19/", "/app-shell-v20/", "verifier cache regex"),
    (
    ...
    if before not in verifier:
        raise SystemExit(f"Missing {label}")
    verifier = verifier.replace(before, after, 1)
...
# ---------------------------------------------------------------------------
Path("tests/autopilot-phase-1.test.js").write_text(
...
print("Applied Clean30 autopilot Phase 1 source migration")
